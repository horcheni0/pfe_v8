...
function Test() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(SummaryApi.conformm.url, {
        method: SummaryApi.conformm.method,
        headers: {
      ...
      <div className="flex-1 max-w-[420px]">
        <form className="login p-6" onSubmit={handleSubmit} >
          ...
          <input value={number}
            ...
            className='w-full border rounded box-border mt-2.5 mb-5 p-2.5 border-solid border-[#ddd]'
            type="number" />
          <label className='dark:text-gray-600'>Address</label>
          <textarea
            value={message}
          ...
          {success && (
            <div className="success">
              Confirmation sent successfully!<br />
              <div className="panel-footer" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', height: '96px', padding: '0 80px', backgroundColor: 'rgb(239, 239, 239)' }}>
    <button className="btn back-btn" style={{ fontSize: '16px', width: '163px', height: '48px', cursor: 'pointer', transition: 'all .2s ease-in-out', letterSpacing: '1px', border: 'none', borderRadius: '23px', color: '#fa0a0a', background: '#fff', outline: 'none' }}><Link to="/cart">Back</Link></button>
    <button type="submit" form="checkoutForm" className="btn next-btn" style={{ fontSize: '16px', width: '163px', height: '48px', cursor: 'pointer', transition: 'all .2s ease-in-out', letterSpacing: '1px', border: 'none', borderRadius: '23px', color: '#fff', background: '#fc1d1d', outline: 'none' }}><Link to="/checkout">Next</Link></button>
</div>
            </div>
          )}
